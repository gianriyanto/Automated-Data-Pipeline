...
from cfgdags.gcs_sensor_trigger_email_rpt import REPORT_CONFIG
...
for itr in report_config:
    rpt_bucket_name = report_config[rpt_folder_cnt]['bucket_name']
    rpt_folder_name = report_config[rpt_folder_cnt]['folder_name']
    rpt_rcpnt = report_config[rpt_folder_cnt]['recipient']
    rpt_pic = report_config[rpt_folder_cnt]['pic']

    additions_to_template = []
    this_rpt_cnt = 0
    for itr_rpt_qry in report_config[rpt_folder_cnt]['reports']:
        rpt_name = report_config[rpt_folder_cnt]['reports'][this_rpt_cnt]['report_name']
        rpt_link = "https://storage.cloud.google.com/%s/%s/%s_%s" % (rpt_bucket_name, rpt_folder_name, "{{ ds }}", rpt_name)

        add_to_template = """
                                  <table border="0" cellpadding="0" cellspacing="0" class="btn btn-primary" style="border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%; box-sizing: border-box;">
                                  <p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;">This is Report <b>{}</b> for period <b>{{{{ ds }}}}</b></p>
                                  <tbody>
                                    <tr>
                                      <td align="left" style="font-family: sans-serif; font-size: 14px; vertical-align: top; padding-bottom: 15px;">
                                        <table border="0" cellpadding="0" cellspacing="0" style="border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: auto;">
                                          <tbody>
                                            <tr>
                                                <td style="font-family: sans-serif; font-size: 14px; vertical-align: top; background-color: #3498db; border-radius: 5px; text-align: center;"> <a href=" {}.txt" target="_blank" style="display: inline-block; color: #ffffff; background-color: #3498db; border: solid 1px #3498db; border-radius: 5px; box-sizing: border-box; cursor: pointer; text-decoration: none; font-size: 14px; font-weight: bold; margin: 0; padding: 12px 25px; text-transform: capitalize; border-color: #3498db;">{}</a> </td>
                                            </tr>
                                          </tbody>
                                        </table>
                                      </td>
                                  ...
                                  """.format(rpt_name, rpt_link, rpt_name)
    ...
    task_start_report = BashOperator(
        task_id='start_' + rpt_bucket_name + '_' + rpt_folder_name,
    ...
    for itr_rpt in report_config[rpt_folder_cnt]['reports']:
        rpt_name = report_config[rpt_folder_cnt]['reports'][rpt_cnt]['report_name']
        ...
        task_gcs_give_acl = GoogleCloudStorageObjectCreateAclEntryOperator(
            ...
            object_name='%s/%s_%s.txt' % (rpt_folder_name, "{{ ds }}", rpt_name),
            entity='allUsers',
            role='READER',
            bucket='rpt_bucket_name',
